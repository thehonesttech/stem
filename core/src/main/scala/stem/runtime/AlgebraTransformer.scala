...
object AlgebraTransformer {
  private def layer[State: Tag, Event: Tag, Reject: Tag](algebra: AlgebraCombinators[State, Event, Reject]) = ZLayer.succeed(algebra)

  private def transform[State: Tag, Event: Tag, Reject: Tag](algebra: AlgebraCombinators[State, Event, Reject]) =
  ...
  def withAlgebra[State: Tag, Event: Tag, Reject: Tag, S[_, _]: scalapb.zio_grpc.TransformableService](
    ...
    algebra: AlgebraCombinators[State, Event, Reject]
  ...
  object Ops {
    implicit class RichAlgebraService[State: Tag, Event: Tag, Reject: Tag, S[_, _]: scalapb.zio_grpc.TransformableService](service: S[ZEnv with Combinators[State, Event, Reject], Any]) {
      def withAlgebra(algebra:  AlgebraCombinators[State, Event, Reject]): S[ZEnv, Any] = {
